 ...
 *
 * 通过 PrintableMaxHeap.treePrint(maxHeap) 的方式调用
 */
public class PrintableMaxHeap {
    ...
    public static void treePrint(MaxHeap<Integer> maxHeap){
        ...
        for( int i = 0 ; i < maxHeap.size() ; i ++ ){
            ...
            assert maxHeap.get(i) >= 0 && maxHeap.get(i) < 100;
        ...
        for( int level = 0 ; level < maxLevel ; level ++ ){
            ...
            for( int indexCurLevel = 0 ; indexCurLevel < curLevelNumber ; index ++ , indexCurLevel ++ ){
                line1 = putNumberInLine( maxHeap.get(index) , line1 , indexCurLevel , curTreeMaxLevelNumber*3-1 , isLeft );
                isLeft = !isLeft;
            }
